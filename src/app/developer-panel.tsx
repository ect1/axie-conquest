...
import { Battle, createBattle, createSandboxArmy } from '@/game/battle';
...
export default function DeveloperPanel({ settings, onSettings, objects, status, ready, onClose, onRegenerate, onRemove, unitStats = DEFAULT_UNIT_GLOBAL_STATS, onUnitStats = stats => setActiveUnitGlobalStats(stats) }: Props) {
  ...
  return <section className="developer panel" aria-label="Developer">
    ...
    {tab === 'battle' && <div><p>Compare formations in a practice fight. Troops and world progress are unaffected.</p><label className="developer-distance">Formation<select value={composition} onChange={e => setComposition(e.target.value as typeof composition)}><option value="balanced">Balanced</option><option value="infantry">Infantry heavy</option><option value="archer">Archer heavy</option></select></label><label className="developer-distance">Enemy troops per squad<input type="number" min={1} max={100} value={enemyCount} onChange={e => setEnemyCount(Math.max(1, Math.min(100, Math.floor(Number(e.target.value) || 1))))} /></label><p>Each preset has one Axie leader and 40 trained troops. Use Ranges / inspect to toggle blue awareness, orange engagement, red attack and white body circles.</p><button className="primary" onClick={() => setPractice(createBattle(createSandboxArmy(composition), enemyCount))}>Open battle sandbox</button></div>}
    {practice && <BattleArena battle={practice} onChange={setPractice} sandbox onReset={() => setPractice(createBattle(createSandboxArmy(composition), enemyCount))} onClose={() => setPractice(null)} />}
    <ResetGameControl />
  </section>;
}
