...
export default function MilitaryPanel({ onClose }: MilitaryPanelProps) {
  ...
  const addDefenseMilitary = () => {
    if (!militaryDraft.military || !Number.isSafeInteger(militaryDraft.militaryCount) || militaryDraft.militaryCount <= 0 || militaryDraft.militaryCount > MILITARY_INFO[militaryDraft.military].available) return;
    setDefenseMilitary({ ...militaryDraft, heroId: null });
  };
  const openDefense = () => {
  ...
  return <section className="military panel" aria-label="Military command">
    ...
    <div className="military-cards">
      ...
      <button className="military-card defense-card" onClick={openDefense}><span className="military-card-icon">▥</span><span><strong>Defense</strong><small>{defender ? '1/1 Axie assigned' : '0/1 Axies assigned'}{defenseMilitary.military ? ` · ${defenseMilitary.military} ${defenseMilitary.militaryCount}` : ''} · Includes walls</small></span><span className="card-chevron">›</span></button>
    ...
    {popup && <div className="assignment-backdrop" role="presentation" onClick={closePopup}><section className={`assignment-popup ${popup === 'offense' ? 'formation-popup' : ''}`} role="dialog" aria-modal="true" aria-label={`Manage ${popup}`} onClick={event => event.stopPropagation()}>
      {popup === 'offense' ? <>
        ...
        <div className="placement-actions">
          ...
          {popup === 'defense' && slotTab === 'military' && <button className="primary" disabled={!militaryDraft.military || militaryDraft.militaryCount <= 0} onClick={addDefenseMilitary}>Update</button>}
        </div>
      </>}
    </section></div>}
  </section>;
