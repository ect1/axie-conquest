...
export default function MilitaryPanel({ onClose }: MilitaryPanelProps) {
  ...
  return <section className="military panel" aria-label="Military command">
    ...
    <div className="military-cards">
      ...
      <button className="military-card offense-card" onClick={openFormation}><span className="military-card-icon">⚔</span><span><strong>Offense</strong><small>Template {template + 1} · Leader {activeFormation.leader ? 'assigned' : 'empty'} · Front / Mid / Back</small></span><span className="card-chevron">›</span></button>
    </div>
    ...
    {popup && <div className="assignment-backdrop" role="presentation" onClick={closePopup}><section className={`assignment-popup ${popup === 'offense' ? 'formation-popup' : ''}`} role="dialog" aria-modal="true" aria-label={`Manage ${popup}`} onClick={event => event.stopPropagation()}>
      {popup === 'offense' ? <>
        <div className="catalog-heading"><div><span className="eyebrow">OFFENSE FORMATION</span><h3>March template {template + 1}</h3><small>4 Front · 5 Mid · 4 Back · 5 Rear · 1 Leader</small></div><button className="close" aria-label="Close formation" onClick={closePopup}>&times;</button></div>
        <div className="template-tabs" role="tablist" aria-label="Formation templates">{[0, 1, 2].map(index => <button key={index} role="tab" aria-selected={template === index} className={template === index ? 'active' : ''} onClick={() => setTemplate(index)}>Template {index + 1}</button>)}</div>
        ...
        <button className="primary assignment-done" onClick={closePopup}>Save template</button>
      </> : <>
        <div className="catalog-heading"><div><span className="eyebrow">{popup === 'slot' ? 'FORMATION SLOT' : 'ASSIGN AXIE'}</span><h3>{popup === 'scout' ? 'Scout position' : popup === 'defense' ? 'Defense position' : popup === 'leader' ? 'Formation leader' : 'Assign slot'}</h3><small>{popup === 'scout' ? `${scouts.length}/3 positions filled` : popup === 'defense' ? `${defender ? 1 : 0}/1 position filled` : popup === 'leader' ? 'Choose from Axies already in this template' : 'Choose an Axie or military units'}</small></div><button className="close" aria-label="Close assignment" onClick={closePopup}>&times;</button></div>
        ...
        <div className="placement-actions">
          <button className="secondary" onClick={() => popup === 'slot' || popup === 'leader' ? setPopup('offense') : closePopup()}>{popup === 'slot' || popup === 'leader' ? 'Back to template' : 'Done'}</button>
