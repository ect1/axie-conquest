...
type Props = { axies: readonly ApiAxie[]; deployedIds: readonly string[]; status: 'loading' | 'ready' | 'error'; error: string; syncedAt: number | null; onDeploy: (id: string) => void; onEnlist: (id: string) => void; onClose: () => void; };

function Portrait({ axie }: { axie: ApiAxie }) {
  return <span className="api-axie-fallback" aria-hidden="true">{axie.name.slice(0, 1)}</span>;
}

export default function HeroesPanel({ axies, deployedIds, status, error, syncedAt, onDeploy, onEnlist, onClose }: Props) {
  const [tab, setTab] = useState<'active' | 'deployed'>('active');
  const [selectedId, setSelectedId] = useState<string | null>(null);
  const deployed = axies.filter(axie => deployedIds.includes(axie.id));
  const active = axies.filter(axie => !deployedIds.includes(axie.id));
