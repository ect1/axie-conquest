import { BattleRangeSettings, isInDirectionalRange } from './battle-range';

export const SANDBOX_BATTLE_STEP = .1;
export const SANDBOX_AXIE_STATS = { speed: 2.5 };
export const SANDBOX_CHIMERA_STATS = { speed: 2.3 };
export type SandboxBattleUnit = { id: string; side: 'player' | 'enemy'; x: number; z: number; facing: number; speed: number; attackRange: number; state: 'marching' | 'holding' | 'approaching' | 'charging' | 'in-range'; targetId: string | null };
...
export function stepSandboxBattle(previous: SandboxBattle, settings: BattleRangeSettings): SandboxBattle {
  ...
  for (const unit of units) {
    ...
    const target = previous.units.filter(candidate => candidate.side !== unit.side)
    ...
    if (!target) { unit.state = 'marching'; unit.targetId = null; unit.x += Math.sin(origin.facing) * unit.speed * SANDBOX_BATTLE_STEP; unit.z += Math.cos(origin.facing) * unit.speed * SANDBOX_BATTLE_STEP; continue; }
